...
const Plugin = () => {
  ...
  function getCodeAttributes(codeDescription: string) {
    const [language, ...configParts] = codeDescription.split(' ');

    if (configParts.length === 0) {
      return {language, attribute: ''};
    }

    const configMatch = configParts.join(' ').match(/\[(.*)\]/);
    if (!configMatch) {
      return {language, attribute: ''};
    }

    return {language, attribute: configMatch[1]};
  }
  ...
  return {
    ...
    init: function (reveal) {
      ...
      if (!renderer) {
        ...
        renderer.code = (code, codeDescription) => {
          const {language, attribute} = getCodeAttributes(codeDescription);
